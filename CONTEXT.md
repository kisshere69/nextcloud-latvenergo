...
| 11 | Техдокументация (docs/TECHNICAL.md) | [x] |
| 12 | Раздел «для продакшена» | [x] |
...
Текущий статус: Задачи 0-12 готовы. Все пункты списка задач закрыты.
...
- Задачи 11/12: `docs/TECHNICAL.md` написан на латышском (по просьбе
  пользователя) — архитектура, ключевые решения, деплой, TLS,
  безопасность, мониторинг, известные ограничения среды, раздел «чего
  не хватает для продакшена» (email, 2FA enforcement, docker.sock в
  certbot-renew, backup, HA, server ID, AppAPI, Grafana не за
  auth/VPN). ВАЖНО: латышский — не родной язык модели, пользователю
  рекомендовано дать документ на вычитку носителю языка перед встречей.
Все 12 задач из списка закрыты. Осталось только то, что руками делает
пользователь перед встречей (см. «Чек-лист перед встречей» выше):
скриншоты Overview/Grafana, вычитка docs/TECHNICAL.md носителем
латышского, прогон ответов «почему не X» вслух.
